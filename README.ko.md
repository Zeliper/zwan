...
한 가지 제약: 네트워크별 주소 변환이 들어가기 전까지는, **같은 오버레이 대역**을 쓰는 두 네트워크가
같은 주소를 배정할 수 있고 호스트의 라우팅 테이블은 하나다. 네트워크마다 대역을 다르게 잡고(`--cidr`),
겹치면 클라이언트가 경고를 표시한다.
...
동작: 컨트롤 플레인 TLS(ACME 또는 지문 핀닝 자체서명), 그룹 ACL, 다중 네트워크, 암호 터널(직접+릴레이), Split-DNS+서비스
레지스트리, L4 라우터, 데스크톱 앱(트레이+서비스+IPC), Windows 설치파일, 자동 업데이트.
남음: 네트워크별 주소 변환(대역 중복 허용), 서비스별 VIP + 전-포트 투명 포워더(TCP+UDP), NAT 트래버설,
IPv6 전송, 코드 서명.

...
- 🧭 **이름 기반 접속** — Split-DNS 리졸버가 `서비스.<서픽스>` 를 올바른 노드로 해석하고, Windows 에 **그 이름만** 리졸버로 보내도록 등록한다. 아무 프로그램에서나 `ping nas.home.zwan` 이 되고, 나머지 이름은 원래 경로 그대로다
...
- 🌏 **한국어·영어** — 창과 트레이가 시스템 언어를 따라가고, 헤더에서 바로 바꿀 수 있다. 주소·토큰·명령어는 번역하지 않는다
...
이름은 Windows 자체에 연결된다. 엔진이 가입한 네트워크마다 이름 해석 정책 규칙을 하나씩 유지해서
`*.home.zwan` 만 로컬 리졸버로 가고 나머지 이름은 원래 DNS 경로를 그대로 탄다 — 일반 인터넷은 건드리지
않는다. 네트워크를 떠나거나 서비스를 멈추면 규칙도 사라지고, 강제 종료된 실행이 남긴 규칙은 다음 기동에서
정리된다. 이름은 기기가 정한 별칭으로 스코프되므로, **같은 서비스 이름이 두 네트워크에서 서로 다른
주소로** 풀린다.
...
번호를 받으므로 둘이 겹칠 수 없다. 다른 멤버 쪽에서는 이름이 전부다 — 게임 서버 주소 칸에
`minecraft.home.zwan` 를 넣으면 된다. 브라우저든 게임이든 탐색기든, 기기의 어느 프로그램에서나 통한다.
...
남음: NAT 트래버설(NAT 양쪽 간 직접 P2P — 지금은 릴레이가 덮는다), IPv6 전송, 코드 서명.
